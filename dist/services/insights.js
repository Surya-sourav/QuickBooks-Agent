...
export const buildSummary = async () => {
    ...
    const monthlyPaymentsRes = await query(`SELECT to_char(date_trunc('month', txn_date), 'YYYY-MM') as month,
            COALESCE(SUM(total_amt),0) as total
     FROM qbo_payments
     WHERE txn_date IS NOT NULL
     GROUP BY 1
     ORDER BY 1`);
    const monthlyJournalRes = await query(`SELECT to_char(date_trunc('month', txn_date), 'YYYY-MM') as month,
            COUNT(*) as count
     FROM qbo_journal_entries
     WHERE txn_date IS NOT NULL
     GROUP BY 1
     ORDER BY 1`);
    const topCustomersRes = await query(`SELECT c.display_name, p.customer_ref, SUM(p.total_amt) as total
     FROM qbo_payments p
     ...
     ORDER BY total DESC
     LIMIT 10`);
    const txnTypeRes = await query(`SELECT txn_type, COUNT(*) as count, COALESCE(SUM(amount),0) as total
