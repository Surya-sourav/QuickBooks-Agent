...
const fallbackResponse = (summary: any): AnalysisResponse => {
  ...
  const chart = {
    ...
    type: "bar",
    data: {
      labels: summary.monthlyPayments.map((m: any) => m.month),
      datasets: [
        {
          label: "Payments",
          data: summary.monthlyPayments.map((m: any) => Number(m.total)),
...
export const runAnalysisAgent = async (question: string): Promise<AnalysisResponse> => {
