...
function parseJournalTotals(entry: any): { debit: number; credit: number } {
  let debit = 0;
  let credit = 0;
...
async function upsertCustomer(customer: any): Promise<void> {
  ...
  if (!id) return;
  await db.query(
    `insert into qbo_customers (id, display_name, active, data)
     values ($1, $2, $3, $4)
     on conflict (id)
     do update set display_name = excluded.display_name,
                   ...
                   data = excluded.data,
                   updated_at = now()`,
    [id, customer.DisplayName ?? customer.FullyQualifiedName ?? null, customer.Active ?? null, customer]
  );
}

async function upsertPayment(payment: any): Promise<void> {
  const id = String(payment.Id ?? "");
  if (!id) return;
  await db.query(
    `insert into qbo_payments (id, txn_date, amount, customer_ref, data)
     values ($1, $2, $3, $4, $5)
     on conflict (id)
...
async function upsertJournalEntry(entry: any): Promise<void> {
  const id = String(entry.Id ?? "");
  if (!id) return;
  const totals = parseJournalTotals(entry);
  await db.query(
    `insert into qbo_journal_entries (id, txn_date, total_debit, total_credit, data)
     values ($1, $2, $3, $4, $5)
     on conflict (id)
     do update set txn_date = excluded.txn_date,
                   ...
                   data = excluded.data,
                   updated_at = now()`,
    [id, entry.TxnDate ?? null, totals.debit, totals.credit, entry]
  );
}
...
async function upsertAccount(account: any): Promise<void> {
  ...
  if (!id) return;
  await db.query(
    `insert into qbo_accounts (id, name, account_type, data)
     values ($1, $2, $3, $4)
     on conflict (id)
     do update set name = excluded.name,
                   ...
                   data = excluded.data,
                   updated_at = now()`,
    [id, account.Name ?? null, account.AccountType ?? null, account]
  );
}

async function storeTransactionList(report: any, params: { startDate: string; endDate: string }): Promise<number> {
  const columns = Array.isArray(report?.Columns?.Column) ? report.Columns.Column : [];
  const columnKeys = columns.map((col: any) => col?.ColTitle ?? col?.ColName ?? "");
  ...
  for (const row of rows) {
    ...
    const amount = toNumber(
      normalized["amount"] ?? normalized["amount_due"] ?? normalized["open_balance"]
    );

    await db.query(
      `insert into qbo_transaction_list_rows (txn_type, txn_date, amount, row_json, report_params)
       values ($1, $2, $3, $4, $5)`,
      [txnType, txnDate, amount, rowObject, params]
    );
...
export async function ingestAll(): Promise<{
  ...
  await db.query(
    `insert into qbo_sync_runs (synced_at, summary)
     values (now(), $1)`,
    [
      {
