...
export const getConnection = async () => {
  const res = await query<QboConnection>("SELECT * FROM qbo_connection ORDER BY id DESC LIMIT 1");
...
export const saveConnection = async (input: {
  ...
  await withClient(async (client) => {
    ...
    if (existing.rows[0]) {
      await client.query(
        "UPDATE qbo_connection SET realm_id=$1, environment=$2, access_token=$3, refresh_token=$4, access_token_expires_at=$5, refresh_token_expires_at=$6, updated_at=NOW() WHERE id=$7",
        [realmId, config.qbo.environment, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt, existing.rows[0].id]
      );
    } else {
      await client.query(
        "INSERT INTO qbo_connection (realm_id, environment, access_token, refresh_token, access_token_expires_at, refresh_token_expires_at) VALUES ($1,$2,$3,$4,$5,$6)",
        [realmId, config.qbo.environment, accessToken, refreshToken, accessTokenExpiresAt, refreshTokenExpiresAt]
      );
    }
  });
};

const isExpired = (iso: string, skewMs = 60_000) => {
...
export const qboGet = async <T>(path: string, params?: Record<string, string>) => {
...
export const qboReport = async <T>(realmId: string, report: string, params: Record<string, string>) => {
  return qboGet<T>(`/v3/company/${realmId}/reports/${report}`, {
    minorversion: config.qbo.minorVersion,
    ...params
  });
};
